...
//
  Future<List<String>>getSyllabusDocIds( ) async {
    ...
    final docRef = await firestore.collection("/NewSchool/G0ITybqOBfCa9vownMXU/attendence/y2Yes9Dv5shcWQl9N9r2/syllabus/10/Syllab").get();
  ...
  Future<List> getSubjectNames() async {
    ...
    final syllabusQuery = await firestore.collection("/NewSchool/G0ITybqOBfCa9vownMXU/attendance/y2Yes9Dv5shcWQl9N9r2/syllabus/10/syllabus/").get();
...
//genericProvider.loggedInStudent.classs
    QuerySnapshot syllabusSnapshot = await FirebaseFirestore.instance
        .collection('/NewSchool/G0ITybqOBfCa9vownMXU/attendance/y2Yes9Dv5shcWQl9N9r2/syllabus')
...
print(subjects);
  ...
  Future<List<Map<String, dynamic>>> fetchStudyMaterial(
      {required String subjectName, required String clazz, required String section}) async {
    final firestore = FirebaseFirestore.instance;
             ...
             Expanded(
               child: ListView(
                 ...
                 children: filteredSubjects.map((subject) {
                   return InkWell(
                     onTap: () async {
                       var list = await fetchStudyMaterial(subjectName: subject,clazz: genericProvider.loggedInStudent.classs, section: genericProvider.loggedInStudent.section);
                       print(list);
                       Navigator.push(
